...
public class Solution {
    ...
    private static List<String> allReadingTasks(List<Task> tasks) {
        return tasks.stream()
                .filter(n -> n.getType() == TaskType.READING)
                .sorted(Comparator.comparing(Task::getCreatedOn))//вариант моей сортировки .sorted((task1, task2) -> task1.getCreatedOn().compareTo(task2.getCreatedOn()))
                .map(Task::getTitle)//пока не смапил заголовки, стрим не работал
                .toList();
